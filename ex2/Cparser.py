...
class Cparser(object):
    ...
    def p_expression(self, p):
        ...
        if len(p) == 2:
            if isinstance(p[1],str):
                p[0] = AST.Expression(None, None, None, p[1])
            else:
                p[0] = AST.Expression(None, None, None, p[1])
        else:
            if p[1] == '(' and p[3] == ')':
    ...
    def p_expr_list_or_empty(self, p):
        """expr_list_or_empty : expr_list
                              | """
        if len(p) == 1:
            p[0] = AST.Expr_list_or_empty()
        else:
            p[0] = AST.Expr_list_or_empty(p[1])

    def p_expr_list(self, p):
        """expr_list : expr_list ',' expression
                     | expression """
        if len(p) > 2:
           p[0] = AST.Expr_list(p[3], p[1])
        else:
    ...
    def p_args_list_or_empty(self, p):
        """args_list_or_empty : args_list
                              | """
        if len(p) == 1:
            p[0] = AST.Args_list_or_empty()
        else:
            p[0] = AST.Args_list_or_empty(p[1])
